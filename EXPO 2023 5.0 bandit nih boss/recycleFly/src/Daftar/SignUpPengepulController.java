...
public class SignUpPengepulController implements Initializable {
    @FXML
    ...
    void handleButtonRegisterUser(ActionEvent event) {
        if (!usernamePendaftar.getText().equals("") && !passPendaftar.getText().equals("")) {
            ArrayList<DataPengepul> listUser = new ArrayList<>();
            ...
            listUser.add(new DataPengepul(username, pass, nama, tetl, jenisKelamin, email));
            String xml = xstream.toXML(listUser);
            try {
                dataUser = new FileOutputStream("DataPengepul.xml");
